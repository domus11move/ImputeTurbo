#'
#'
#' Introducir de una manera aleatoria valores missings
#'
#' @param data una matriz da data para simular
#' @param p el porcentaje de missing que se introducira a la data, esto será en el rango de 0 a 1
#'
#' @keywords simulation, imputation
#' @return el mismo tamaño de la matriz con los valores missing simulados
#' @export
#' @examples
#' # Crea una data sin los missing como ejemplo
#' simdata <- matrix(rnorm(100), 10, 10)
#'
#' # Introduce los missing con un p de 0.15
#' missingdata <- SimIm(simdata, p = 0.15)
#'
#' # cuenta el numero de missing values
#' sum(is.na(missingdata))
#'
#' #------------------
#'
#' # No hay datos faltantes en la data original de parkinson
#' data(parkinson)
#'
#' # Introduzcamos ahora datos faltantes
#' missdata <- SimIm(parkinson, 0.1)
#'
#' # Cuenta ahora el numero de missings en la nueva data
#' sum(is.na(missdata))
...
#' calcula el RMSE o NRMSE
#'
#' This function calculate imputation error given the imputed data, the missing
#' data and the true data
#'
#' @param imp la data imputada
#' @param mis la data con missings
#' @param true la verdadera data matriz
#' @param norm logical, si eS TRUE ENTONCES EL RMSE volverá
#' @return the RMSE or NRMSE
#' @seealso \code{\link{impute}} por la funcion de imputacion,
#' \code{\link{mr}} pr el error de clasificar mal el error metrico
#' @export
#' @examples
#' data(parkinson)
